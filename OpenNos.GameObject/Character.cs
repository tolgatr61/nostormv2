...
#warning TODO: Change sheep score
        ...
        public string GenerateCInfo() => $"c_info {(Authority > AuthorityType.User && !Undercover ? Authority == AuthorityType.GameSupport ? $"[{Authority}]" + Name : Name : Authority == AuthorityType.BitchNiggerFaggot ? Name + "[BitchNiggerFaggot]" : Name)} - -1 {(Family != null && FamilyCharacter != null && !Undercover ? $"{Family.FamilyId} {Family.Name}({Language.Instance.GetMessageFromKey(FamilyCharacter.Authority.ToString().ToUpper())})" : "-1 -")} {CharacterId} {(Invisible && Authority >= AuthorityType.TrialModerator ? 6 : 0)} {(byte)Gender} {(byte)HairStyle} {(byte)HairColor} {(byte)Class} {(GetDignityIco() == 1 ? GetReputationIco() : -GetDignityIco())} {(/*Authority > AuthorityType.User && !Undercover ? CharacterHelper.AuthorityColor(Authority) : */Compliment)} {(UseSp || IsVehicled ? Morph : 0)} {(Invisible ? 1 : 0)} {Family?.FamilyLevel ?? 0} {(UseSp ? MorphUpgrade : 0)} {ArenaWinner} {Titles}";

        //public string GenerateCInfo() => $"c_info {(/*Authority > AuthorityType.User && !Undercover ? */Authority != AuthorityType.User ? $"[{Authority}]" + Name /*: Name */: Authority == AuthorityType.BitchNiggerFaggot ? Name + "[BitchNiggerFaggot]" : Name)} - -1 {(Family != null && FamilyCharacter != null && !Undercover ? $"{Family.FamilyId} {Family.Name}({Language.Instance.GetMessageFromKey(FamilyCharacter.Authority.ToString().ToUpper())})" : "-1 -")} {CharacterId} {(/*Invisible && Authority == AuthorityType.TMOD ? 6 : */0)} {(byte)Gender} {(byte)HairStyle} {(byte)HairColor} {(byte)Class} {(GetDignityIco() == 1 ? GetReputationIco() : -GetDignityIco())} {(/*Authority > AuthorityType.User && !Undercover ? CharacterHelper.AuthorityColor(Authority) : */Compliment)} {(UseSp || IsVehicled ? Morph : 0)} {(Invisible ? 1 : 0)} {Family?.FamilyLevel ?? 0} {(UseSp ? MorphUpgrade : 0)} {ArenaWinner} {Titles}";
        ...
        public string GenerateEq()
        {
            ...
            return $"eq {CharacterId} {(Invisible && Authority >= AuthorityType.TrialGameMaster ? 6 : Undercover ? (byte)AuthorityType.User : Authority < AuthorityType.User ? (byte)AuthorityType.User : Authority >= AuthorityType.TrialGameMaster ? 2 : (byte)Authority)} {(byte)Gender} {(byte)HairStyle} {color} {(byte)Class} {GenerateEqListForPacket()} {(!InvisibleGm ? GenerateEqRareUpgradeForPacket() : null)}";
        ...
        public string GenerateEvtB() => $"evtb 0 0 0 0 0 0 {ServerManager.Instance.Configuration.RateXP} {ServerManager.Instance.Configuration.RateGold} {ServerManager.Instance.Configuration.RateReputation} {ServerManager.Instance.Configuration.RateDrop} 0 0";
        ...
        public string GenerateIn(bool foe = false, AuthorityType receiverAuthority = AuthorityType.User, int InEffect = 0)
        {
            ...
            if (receiverAuthority >= AuthorityType.TrialGameMaster)
            {
                foe = false;
                name = $"[{Faction}]{name}";
            }

            if (foe && Authority < AuthorityType.TrialGameMaster)
            {
                name = "!§$%&/()=?*+~#";
            }
        ...
        public void GenerateKillBonus(MapMonster monsterToAttack, BattleEntity Killer)
        {
            ...
            lock (_syncObj)
            {
                ...
                /* Give un item via un compteur de drop.
        ...
        public string GenerateTit() => $"tit {Language.Instance.GetMessageFromKey(Class == (byte)ClassType.Adventurer ? nameof(ClassType.Adventurer).ToUpper() : Class == ClassType.Swordsman ? nameof(ClassType.Swordsman).ToUpper() : Class == ClassType.Archer ? nameof(ClassType.Archer).ToUpper() : Class == ClassType.Magician ? nameof(ClassType.Magician).ToUpper() : nameof(ClassType.MartialArtist).ToUpper())} {Name}";
            ...
            if (Level >= 70 && referrerId != 0 && !CharacterId.Equals(referrerId))
            {
                ...
                if (logs.Count <= 5)
                {
                    ...
                    if (referrer != null && !AccountId.Equals(character.AccountId))
                    {
                       ...
                       // SendGift(CharacterId, itemvnum, amount, 0, 0, 0, false);
                        //SendGift(referrerId, itemvnum, amount, 0, 0, 0, false);
                    }
                }
            }
        }
        ...
        private void GenerateXp(MapMonster monster, Dictionary<BattleEntity, long> damageList = null, bool isGroupMember = false)
        {
            ...
            if (!Session.Account.PenaltyLogs.Any(s => s.Penalty == PenaltyType.BlockExp && s.DateEnd > DateTime.Now))
            {
                ...
                {
                    HeroXp += (int)((GetHXP(monster, grp) * (isMonsterOwner ? 1 : 0.8f) * (1 + (GetBuff(CardType.Item, (byte)AdditionalTypes.Item.EXPIncreased)[0] / 100D))));
                }
        ...
        public bool RemoveSp(short vnum, bool forced)
        {
            if (Session?.HasSession == true && (!IsVehicled || forced))
            {
                ...
                if (Authority >= AuthorityType.TrialModerator || forced)
                {
                    SpCooldown = 0;
                }
                if (SpCooldown > 0)
