        ...
        public void ChangeMapInstance(long characterId, Guid mapInstanceId, int? mapX = null, int? mapY = null, bool noAggroLoss = false)
        {
            ...
            if (session?.Character != null && !session.Character.IsChangingMapInstance)
            {
                ...
                try
                {
                    ...
                    //TODO : Replace hardcoded npc dialogs 
                    /*int rnd = RandomNumber(0, 120);

                  
                    if (session.CurrentMapInstance.Map.MapId == 1)
                    {
                        Observable.Timer(TimeSpan.FromSeconds(rnd + 8)).Subscribe(observer =>
                        session.SendPacket("say 2 1857 0 Attaque !! Poule mouillée !!"));
                        Observable.Timer(TimeSpan.FromSeconds(rnd + 16)).Subscribe(observer =>
                        session.SendPacket("say 2 1858 0 Aaaaaahhhh....."));
                        Observable.Timer(TimeSpan.FromSeconds(rnd + 25)).Subscribe(observer =>
                        session.SendPacket("say 2 1858 0 On y va !"));
                        Observable.Timer(TimeSpan.FromSeconds(rnd + 34)).Subscribe(observer =>
                        session.SendPacket("say 2 1862 0 Attaque !! Poule mouillée !!"));
                        Observable.Timer(TimeSpan.FromSeconds(rnd + 45)).Subscribe(observer =>
                        session.SendPacket("say 2 1861 0 Aaaaaahhhh....."));
                        Observable.Timer(TimeSpan.FromSeconds(rnd + 56)).Subscribe(observer =>
                        session.SendPacket("say 2 1854 0 On y va !"));
                        Observable.Timer(TimeSpan.FromSeconds(rnd + 20)).Subscribe(observer =>
                        session.SendPacket("say 2 1855 0 Attaque !! Poule mouillée !!"));
                    }
                    if (session.CurrentMapInstance.Map.MapId == 5)
                    {
                        Observable.Timer(TimeSpan.FromSeconds(rnd)).Subscribe(observer =>
                        session.SendPacket("say 2 2883 0 On y va !"));
                        Observable.Timer(TimeSpan.FromSeconds(rnd + 7)).Subscribe(observer =>
                        session.SendPacket("say 2 2884 0 Misérable Koaren !"));
                        Observable.Timer(TimeSpan.FromSeconds(rnd + 17)).Subscribe(observer =>
                        session.SendPacket("say 2 2887 0 Tenez vous prêts!"));
                        Observable.Timer(TimeSpan.FromSeconds(rnd + 27)).Subscribe(observer =>
                        session.SendPacket("say 2 2889 0 Quand est-ce que cela se finira ?"));
                        Observable.Timer(TimeSpan.FromSeconds(rnd + 34)).Subscribe(observer =>
                        session.SendPacket("say 2 2889 0 J'espère que ce conflit cessera un jour !"));
                        Observable.Timer(TimeSpan.FromSeconds(rnd + 40)).Subscribe(observer =>
                        session.SendPacket("say 3 2799 0 Victoire !! Victoire !!"));
                        Observable.Timer(TimeSpan.FromSeconds(rnd + 2)).Subscribe(observer =>
                        session.SendPacket("say 3 2799 0 Kaoren ne te pardonnera jamais !!!"));
                        Observable.Timer(TimeSpan.FromSeconds(rnd)).Subscribe(observer =>
                        session.SendPacket("say 3 2796 0 Nous vaincrons !! Victoire !!"));
                        Observable.Timer(TimeSpan.FromSeconds(rnd + 13)).Subscribe(observer =>
                        session.SendPacket("say 3 2807 0 Pour les Koarens !"));
                        Observable.Timer(TimeSpan.FromSeconds(rnd)).Subscribe(observer =>
                        session.SendPacket("say 3 2813 0 Les Kovolts sont libres !"));
                        Observable.Timer(TimeSpan.FromSeconds(rnd + 21)).Subscribe(observer =>
                        session.SendPacket("say 3 2845 0 Nous vaincrons, VICTOIRE ! !"));
                        Observable.Timer(TimeSpan.FromSeconds(rnd + 42)).Subscribe(observer =>
                        session.SendPacket("say 3 2833 0 Victoire !! Victoire !!"));
                        Observable.Timer(TimeSpan.FromSeconds(rnd + 33)).Subscribe(observer =>
                        session.SendPacket("say 3 2804 0 Pour les koarens !"));
                        Observable.Timer(TimeSpan.FromSeconds(rnd + 11)).Subscribe(observer =>
                        session.SendPacket("say 3 2802 0 Victoire !! Victoire !!"));
                    }
                    if (session.CurrentMapInstance.Map.MapId == 6)
                    {
                        Observable.Timer(TimeSpan.FromSeconds(rnd)).Subscribe(observer =>
                        session.SendPacket("say 3 2902 0 Personne n'a à nous dire quoi faire !"));
                        Observable.Timer(TimeSpan.FromSeconds(rnd + 5)).Subscribe(observer =>
                        session.SendPacket("say 2 2945 0 *Murmures...*"));
                        Observable.Timer(TimeSpan.FromSeconds(rnd + 15)).Subscribe(observer =>
                        session.SendPacket("say 3 2910 0 Pourquoi vous nous torturez ???"));
                        Observable.Timer(TimeSpan.FromSeconds(rnd + 30)).Subscribe(observer =>
                        session.SendPacket("say 3 2918 0 Tu oses te frotter à nous ? !"));
                        Observable.Timer(TimeSpan.FromSeconds(rnd + 40)).Subscribe(observer =>
                        session.SendPacket("say 3 2902 0 Tu oses te frotter à nous ? !"));
                    }
                    if (session.CurrentMapInstance.Map.MapId == 20)
                    {
                        Observable.Timer(TimeSpan.FromSeconds(rnd)).Subscribe(observer =>
                        session.SendPacket("say 2 3945 0 Les humains nous tuent sans merci !"));
                        Observable.Timer(TimeSpan.FromSeconds(rnd + 10)).Subscribe(observer =>
                        session.SendPacket("say 2 3945 0 Améliorez les conditions de travail !!!"));
                        Observable.Timer(TimeSpan.FromSeconds(rnd + 25)).Subscribe(observer =>
                        session.SendPacket("say 2 3947 0 Grrrrrr ! Koaren !"));
                    }
                    if (session.CurrentMapInstance.Map.MapId == 261)
                    {
                        Observable.Timer(TimeSpan.FromSeconds(rnd)).Subscribe(observer =>
                        session.SendPacket("say 2 13417 0 Un étranger traîne dans les parages !"));
                        Observable.Timer(TimeSpan.FromSeconds(rnd + 20)).Subscribe(observer =>
                        session.SendPacket("say 2 13401 0 Éloignes - toi de moi !!!"));
                        Observable.Timer(TimeSpan.FromSeconds(rnd + 40)).Subscribe(observer =>
                        session.SendPacket("say 2 13401 0 Nous devons attraper cet étranger !"));
                    }*/
        ...
        private void MaintenanceProcess()
        {
            ...
            if (maintenanceLog != null)
            {
                if (maintenanceLog.DateStart <= DateTime.Now)
                {
                    ...
                    sessions.Where(s => s.Account.Authority < AuthorityType.SuperModerator).ToList().ForEach(session => session.Disconnect());
                }
                ...
                switch (message.Type)
                {
                    ...
                    case MessageType.Whisper:
                        ...
                        if (targetSession.Character.GmPvtBlock)
                        {
                            ...
                            && DAOFactory.AccountDAO.LoadById(DAOFactory.CharacterDAO.LoadById(message.SourceCharacterId).AccountId).Authority < AuthorityType.TrialModerator)
                        {
                            if (message.DestinationCharacterId != null)
                            {
                                CommunicationServiceClient.Instance.SendMessageToCharacter(new SCSCharacterMessage
