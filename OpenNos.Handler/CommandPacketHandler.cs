...
namespace OpenNos.Handler
{
    public class CommandPacketHandler : IPacketHandler
    {
       ...
       /* public void AddTitle(AddTitlePacket addTitlePacket)
        {
            Logger.LogUserEvent("GMCOMMAND", Session.GenerateIdentity(), $"[AddTitle]TitleVNum: {addTitlePacket.TitleVnum} TitleId: {addTitlePacket.TitleId}");

            if (DAOFactory.CharacterTitleDAO.LoadByVNum(addTitlePacket.TitleVnum, Session.Character.CharacterId) != null)
            {
                Session.SendPacket(Session.Character.GenerateSay("Title already exist!", 11));
                return;
            }
            Session.Character.AddTitle(addTitlePacket.TitleVnum, addTitlePacket.TitleId);
        }*/
        ...
        public void CHConnect(CHPacket chpacket)
        {
            ...
            {
                Session.SendPacket(Session.Character.GenerateSay(Language.Instance.GetMessageFromKey("NOT_ENOUGH_MONEY_CH_COST"), 10));
                return;
            }

            Session.Character.Gold = Session.Character.Gold - 5000;
            switch (chpacket.ch)
            {
                case 1:
                    Session.Character.ChangeChannel("25.81.119.156", 3438, 3);
                    break;
                case 2:
                    Session.Character.ChangeChannel("25.81.119.156", 3439, 3);
                    break;
                case 3:
                    Session.Character.ChangeChannel("25.81.119.156", 3440, 3);
                    break;
                case 4:
                    Session.Character.ChangeChannel("25.81.119.156", 3441, 3);
                    break;
                case 5:
                    Session.Character.ChangeChannel("25.81.119.156", 3442, 3);
                    break;
            }
        }
                                     ...
                                     "all", StringComparison.OrdinalIgnoreCase)))
                            {
                                ...
                                else if (amount <= Session.Character.Gold && Session.Character.Gold > 0)
                                {
                                    if (amount < 1)
                                    {
                                        ...
                                        Session.SendPacket(Session.Character.GenerateSay(Language.Instance.GetMessageFromKey("CHEAT_TRY_LOGGED"), 10));
                                    }
                                    else
                                    {
                                        Logger.LogEvent("BANK",
        ...
        public void Command(HelpPacket helpPacket)
        {
            ...
            List<Type> classes = AppDomain.CurrentDomain.GetAssemblies().SelectMany(t => t.GetTypes()).Where(t =>
                ...
                && (((PacketHeaderAttribute)Array.Find(t.GetCustomAttributes(true),
                     ...
                     || c.Equals(AuthorityType.TrialModerator) && Session.Account.Authority >= AuthorityType.TrialModerator && Session.Account.Authority <= AuthorityType.BoardAdmin
                     || c.Equals(AuthorityType.Moderator) && Session.Account.Authority >= AuthorityType.Moderator && Session.Account.Authority <= AuthorityType.BoardAdmin
                     || c.Equals(AuthorityType.SuperModerator) && Session.Account.Authority >= AuthorityType.SuperModerator && Session.Account.Authority <= AuthorityType.BoardAdmin
                     || c.Equals(AuthorityType.TrialGameMaster) && Session.Account.Authority >= AuthorityType.TrialGameMaster
                     || c.Equals(AuthorityType.GameMaster) && Session.Account.Authority >= AuthorityType.GameMaster
                     || c.Equals(AuthorityType.SuperGameMaster) && Session.Account.Authority >= AuthorityType.SuperGameMaster
                     || c.Equals(AuthorityType.GameAdmin) && Session.Account.Authority >= AuthorityType.GameAdmin
                     || c.Equals(AuthorityType.TeamManager) && Session.Account.Authority >= AuthorityType.TeamManager
                     || c.Equals(AuthorityType.CommunityManager) && Session.Account.Authority >= AuthorityType.CommunityManager
                        ...
                        if (Session.Account.Authority >= account?.Authority)
                        {
                            ...
                            switch (account.Authority)
                            {
                                case AuthorityType.Developper:
                                    newAuthority = AuthorityType.CommunityManager;
                                    break;
                                case AuthorityType.CommunityManager:
                                    newAuthority = AuthorityType.TeamManager;
                                    break;
                                case AuthorityType.TeamManager:
                                    newAuthority = AuthorityType.GameAdmin;
                                    break;
                                case AuthorityType.GameAdmin:
                                    newAuthority = AuthorityType.SuperGameMaster;
                                    break;
                                case AuthorityType.SuperGameMaster:
                                    newAuthority = AuthorityType.GameMaster;
                                    break;
                                case AuthorityType.GameMaster:
                                    newAuthority = AuthorityType.GameSupport;
                                    break;
        ...
        public void DropRate(DropRatePacket dropRatePacket)
        {
            if (dropRatePacket != null)
            {
                ...
                if (dropRatePacket.Value <= 1000)
                {
                    ...
                    ServerManager.Instance.Broadcast(Session, Session.Character.GenerateEvtB(),ReceiverType.All);
                }
                else
                {
                    Session.SendPacket(
                        ...
                        if (account.Authority < Session.Account.Authority)
                        {
                            ...
                            switch (account.Authority)
                            {
                                ...
                                case AuthorityType.TeamManager:
                                    ...
                                    break;
                                case AuthorityType.CommunityManager:
                                    newAuthority = AuthorityType.Developper;
                                    break;
                                default:
        ...
        /// $SearchMap Command
        /// </summary>
        /// <param name="searchMapPacket"></param>
        public void SearchMap(SearchMapPacket searchMapPacket)
        {
            if (searchMapPacket != null)
            {
                string contents = searchMapPacket.Contents;
                Logger.LogUserEvent("GMCOMMAND", Session.GenerateIdentity(),
                    $"[SearchMap]Contents: {(string.IsNullOrEmpty(contents) ? "none" : contents)}");
                ...
                IEnumerable<MapDTO> maplist = DAOFactory.MapDAO.FindByName(name)
                    .OrderBy(s => s.MapId).Skip(page * 200).Take(200).ToList();
                if (maplist.Any())
                {
                    foreach (MapDTO map in maplist)
                    {
                        Session.SendPacket(Session.Character.GenerateSay(
                            $"[SearchMap:{page}]Map: {(string.IsNullOrEmpty(map.Name) ? "none" : map.Name)} VNum: {map.MapId}",
                            12));
                    }
                }
                else
                {
                    Session.SendPacket(
                        Session.Character.GenerateSay(Language.Instance.GetMessageFromKey("MAP_NOT_FOUND"), 11));
                }
            }
            else
            {
        ...
        {
            if (homePacket != null && Session.Character != null)
            {
                //Test it tommorrow
                if (Session.Character.MapId > 5000)
                {
                    return;
                }
                if (Session.Character.Channel.ChannelId != 51)
                {
                    ...
                    ServerManager.Instance.ChangeMap(Session.Character.CharacterId, 1, (short)(79 + ServerManager.RandomNumber(-6, 6)), (short)(117 + ServerManager.RandomNumber(-5, 5)));
                }
                else
                {
                    Session.SendPacket(Session.Character.GenerateSay(Language.Instance.GetMessageFromKey("NOT_POSSIBLE_IN_ACT4"), 10));
                }
            }
            else
            {
