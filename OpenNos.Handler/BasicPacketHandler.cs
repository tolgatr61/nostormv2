    ...
    public class BasicPacketHandler : IPacketHandler
    {
        ...
        public void BlacklistAdd(BlInsPacket blInsPacket)
        {
            ...
            if (DAOFactory.CharacterDAO.LoadById(blInsPacket.CharacterId) is CharacterDTO character
             && DAOFactory.AccountDAO.LoadById(character.AccountId).Authority >= AuthorityType.TrialModerator)
            {
        ...
        public void Say(SayPacket sayPacket)
        {
            ...
            else
            {
                ...
                if (Session.Character.Authority >= AuthorityType.GameSupport)
                {
                ...
                {
                    ArenaTeamMember member2 = member.FirstOrDefault(o => o.Session == Session);
                    member.Where(s => s.ArenaTeamType == member2?.ArenaTeamType && s != member2).ToList().ForEach(o => o.Session.SendPacket(Session.Character.GenerateSay(message.Trim(), type, Session.Account.Authority >= AuthorityType.GameSupport)));
                }
                else if (ServerManager.Instance.ChannelId == 51 && Session.Account.Authority < AuthorityType.TrialModerator)
                {
                ...
                {
                    Session.CurrentMapInstance?.Broadcast(Session, Session.Character.GenerateSay(message.Trim(), type, Session.Character.Authority >= AuthorityType.GameSupport), ReceiverType.AllExceptMe);
                }
            }
        ...
        public void StartGame(GameStartPacket gameStartPacket)
        {
            ...
            switch (Session.Character.Authority)
            {
               /* case AuthorityType.SuperGameMaster:
                ...
                case AuthorityType.CoOwner:
                    Session.Character.Undercover = true;
                    Session.Character.Compliment = 350;
                    break;
            ...
            {
                Session.SendPacket(Session.Character.GenerateSay($"Bienvenue {Session.Character.Name} sur NosTorm V2 ! Developpé par Rayshon.", 11));
            }
            ...
            try
            {
                ...
                if (receiver != null)
                {
                    ...
                    if (receiverSession?.CurrentMapInstance?.Map.MapId == Session.CurrentMapInstance?.Map.MapId
                        && Session.Account.Authority >= AuthorityType.TrialModerator)
                    {
