...
namespace OpenNos.DAL.DAO
{
    public class FamilyDAO : IFamilyDAO
    {
        ...
        /// </summary>
        /// <returns></returns>
       /* public List<FamilyDTO> GetTopFamily()
        {
            using (OpenNosContext context = DataAccessHelper.CreateContext())
            {
                List<FamilyDTO> result = new List<FamilyDTO>();
                foreach (Family entity in context.Family.Where(f => f.FamilyFaction == 1 && f.FamilyFaction == 2  && t => t.OrderByDescending(t => t.TopFamily).Take(2)))
                {
